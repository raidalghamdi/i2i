...
function identityProvider(activeRole: string | null = 'submitter') {
  return {
...
describe('MyIdeasComponent', () => {
  ...
  async function setup(queryParams: Record<string, string> = {}, activeRole: string | null = 'submitter'): Promise<void> {
  ...
  it('hides the Withdraw button for a team-member (non-owner) idea', async () => {
    const teamItem: MyIdeaItem = {
      id: 'i9', code: 'IDEA-009', titleAr: 'فكرة الفريق', titleEn: 'Team Idea',
      status: 'submitted', currentStage: 1, createdAt: '2026-01-01T00:00:00Z',
      updatedAt: '2026-01-05T00:00:00Z', feedbackCount: 0, isOwner: false,
    };
    ideasApi = jasmine.createSpyObj('IdeasApiService', ['getMineDetailed', 'withdraw']);
    ideasApi.getMineDetailed.and.returnValue(Promise.resolve([teamItem]));
    ...
    expect(withdrawButtons.length).toBe(0);
  });
  ...
  it('hides the New idea and empty-state CTAs for a non-author role (e.g. evaluator)', async () => {
    ideasApi = jasmine.createSpyObj('IdeasApiService', ['getMineDetailed', 'withdraw']);
    ideasApi.getMineDetailed.and.returnValue(Promise.resolve([]));
    ...
    const el = fixture.nativeElement as HTMLElement;
    const hrefs = Array.from(el.querySelectorAll('a')).map((a) => a.getAttribute('href'));
    expect(hrefs.some((h) => h?.includes('/ideas/new'))).toBeFalse();
  });
