...
describe('IdeaSubmitWizardComponent', () => {
  ...
  let directoryApi: jasmine.SpyObj<DirectoryApiService>;
  let publicContentApi: jasmine.SpyObj<PublicContentApiService>;
  let router: jasmine.SpyObj<Router>;

  const lhassan: DirectoryPerson = { samAccountName: 'lhassan', displayName: 'Layla Hassan', email: 'lhassan@gac-demo.sa' };
  const ofarouk: DirectoryPerson = { samAccountName: 'ofarouk', displayName: 'Omar Farouk', email: 'ofarouk@gac-demo.sa' };
  ...
  function setup(): void {
    ...
    directoryApi = jasmine.createSpyObj('DirectoryApiService', ['search']);
    publicContentApi = jasmine.createSpyObj('PublicContentApiService', ['getBySlug']);
    publicContentApi.getBySlug.and.returnValue(Promise.resolve(null));
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: PublicContentApiService, useValue: publicContentApi },
  ...
  it('team participation requires a team name and 1-4 members selected from the directory', async () => {
    setup();
    await initialize();
    ...
    fixture.componentInstance.onTeamMembersSelected([lhassan]);
    expect(fixture.componentInstance.stepValid(0)).toBe(false);

    fixture.componentInstance.form.controls.basics.patchValue({ teamName: 'Team A' });
    expect(fixture.componentInstance.stepValid(0)).toBe(true);

    fixture.componentInstance.onTeamMembersSelected([]);
    expect(fixture.componentInstance.stepValid(0)).toBe(false);

    fixture.componentInstance.onTeamMembersSelected([lhassan, ofarouk]);
    expect(fixture.componentInstance.stepValid(0)).toBe(true);
  });

  it('onTeamMembersSelected maps selected directory people into TeamMemberInput with the AD-authoritative name/email', async () => {
    setup();
    await initialize();

    fixture.componentInstance.onTeamMembersSelected([lhassan, ofarouk]);

    expect(fixture.componentInstance.teamMembers()).toEqual([
      { samAccountName: 'lhassan', name: 'Layla Hassan', email: 'lhassan@gac-demo.sa' },
      { samAccountName: 'ofarouk', name: 'Omar Farouk', email: 'ofarouk@gac-demo.sa' },
    ]);
  });
  ...
  it('step 1 (details) is invalid until title, description, problem statement, and expected benefits are filled', async () => {
    setup();
    await initialize();
    ...
    fixture.componentInstance.form.controls.details.patchValue({
  ...
  function fillValidWizard(fixture: ComponentFixture<IdeaSubmitWizardComponent>): void {
    fixture.componentInstance.form.controls.basics.patchValue({ activityId: 'activity-1', strategicThemeId: 'theme-1' });
    fixture.componentInstance.form.controls.details.patchValue({
      title: 'My idea', description: 'A description.',
      problemStatementAr: 'مشكلة',
      expectedBenefitsAr: 'فوائد',
    });
    fixture.componentInstance.queuedFiles.set([makeFile('a.pdf', 'application/pdf', 100)]);
  ...
  it('clicking the "terms and conditions" link opens the terms modal without checking the terms checkbox, and closing it clears the flag', async () => {
    setup();
    await initialize();
    fixture.componentInstance.currentStep.set(3);
    fixture.detectChanges();

    expect(fixture.componentInstance.termsModalOpen()).toBe(false);
    expect(fixture.nativeElement.querySelector('app-terms-modal')).toBeNull();

    const link = Array.from((fixture.nativeElement as HTMLElement).querySelectorAll('button')).find(
      (btn) => btn.textContent?.trim() === 'terms and conditions',
    ) as HTMLButtonElement;
    expect(link).toBeTruthy();
    link.click();
    fixture.detectChanges();

    expect(fixture.componentInstance.termsModalOpen()).toBe(true);
    expect(fixture.nativeElement.querySelector('app-terms-modal')).toBeTruthy();
    expect(fixture.componentInstance.form.controls.review.controls.termsAgreed.value).toBe(false);

    fixture.componentInstance.closeTermsModal();
    fixture.detectChanges();
  ...
  it('onSubmit creates the idea, uploads the queued file, submits it, and navigates to the detail page', async () => {
    ...
    expect(ideasApi.create).toHaveBeenCalledWith(jasmine.objectContaining({
      ...
      problemStatementAr: 'مشكلة', problemStatementEn: 'مشكلة',
      expectedBenefitsAr: 'فوائد', expectedBenefitsEn: 'فوائد',
