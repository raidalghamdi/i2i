...
/** Summary returned by the bulk "import all users from AD" operation. */
export interface AdImportAllResult {
  total: number;
  granted: number;
  pending: number;
...
export class RosterApiService {
  ...
  /** Bulk-import every Active Directory user, granting the submitter role. Admin-only. */
  importAllFromAd(): Promise<AdImportAllResult> {
    return firstValueFrom(this.http.post<AdImportAllResult>('/api/admin/directory/import-all', {}));
  }
}
