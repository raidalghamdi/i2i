...
describe('PostProgramApiService', () => {
  ...
  it('advances an idea to a stage', async () => {
    const promise = service.advance('i1', 'in_pilot', 'Pilot kicked off successfully.');
    const req = httpMock.expectOne('/api/admin/ideas/i1/post-program-stage');
    expect(req.request.method).toBe('POST');
    expect(req.request.body).toEqual({ stage: 'in_pilot', comment: 'Pilot kicked off successfully.' });
    req.flush({ id: 'i1', status: 'in_pilot' });
    expect(await promise).toEqual({ id: 'i1', status: 'in_pilot' });
  });

  it('gets post-program history', async () => {
    const entry = { fromStage: 'approved', toStage: 'in_pilot', comment: 'Kickoff.', changedAt: '2026-01-01', changedById: 'user-1' };
    const promise = service.getHistory('i1');
    const req = httpMock.expectOne('/api/admin/ideas/i1/post-program-history');
    expect(req.request.method).toBe('GET');
    req.flush([entry]);
