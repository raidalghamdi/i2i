...
describe('AppShellComponent', () => {
  ...
  it('shows the header "Submit an idea" button for a submitter', () => {
    setup({
      loadFailed: false,
      identity: { samAccountName: 'submitter1', email: null, department: 'Innovation', roles: ['submitter'], activeRole: 'submitter' },
    });
    expect((fixture.nativeElement as HTMLElement).querySelector('header a[href="/ideas/new"]')).not.toBeNull();
  });

  it('hides the header "Submit an idea" button for an admin (submitter-only quick action)', () => {
    setup({
      loadFailed: false,
      identity: { samAccountName: 'admin1', email: null, department: 'Innovation', roles: ['admin'], activeRole: 'admin' },
    });
    expect((fixture.nativeElement as HTMLElement).querySelector('header a[href="/ideas/new"]')).toBeNull();
  });
  ...
  it('renders both the program logo and the GAC logo in the header', () => {
    ...
    const imgs = Array.from((fixture.nativeElement as HTMLElement).querySelectorAll('header img')) as HTMLImageElement[];
