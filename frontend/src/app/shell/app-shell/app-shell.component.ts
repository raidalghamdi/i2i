...
// Each sidebar section belongs to exactly one role; admin is added to every section so an
// administrator can reach all of them. Gating is by the *active* role (the role switcher), so a
// user with several roles sees the section for whichever role they've switched to.
const IDEAS_ROLES = ['submitter', 'admin'];
const EVALUATION_ROLES = ['evaluator', 'admin'];
const SUPERVISION_ROLES = ['supervisor', 'admin'];
const COMMITTEE_ROLES = ['judge', 'admin'];
const ADMINISTRATION_ROLES = ['admin'];
// The header "Submit an idea" quick-action is a normal-user convenience — shown to submitters only
// (admins can still create ideas via /ideas/new, but don't need the top-nav shortcut).
const SUBMITTER_ONLY = ['submitter'];

@Component({
  selector: 'app-app-shell',
  imports: [RouterOutlet, RouterLink, RouterLinkActive, NgTemplateOutlet, RoleSwitcherComponent, DevUserSwitcherComponent, IconComponent, LoadingStateComponent, ErrorStateComponent, HeaderSearchComponent, NotificationBellComponent],
...
export class AppShellComponent implements OnDestroy {
  ...
  readonly navGroups = computed<NavGroup[]>(() => {
    ...
    if (this.hasAnyRole(IDEAS_ROLES)) {
      groups.push({
    ...
    if (this.hasAnyRole(EVALUATION_ROLES)) {
      groups.push({
    ...
    if (this.hasAnyRole(SUPERVISION_ROLES)) {
      groups.push({
    ...
    if (this.hasAnyRole(COMMITTEE_ROLES)) {
      groups.push({
    ...
    if (this.hasAnyRole(ADMINISTRATION_ROLES)) {
      groups.push({
        ...
        items: [
          ...
          { label: $localize`:@@navAdminHomepage:Homepage`, href: '/admin/home-builder', icon: 'image' },
          { label: $localize`:@@navAdminIdeaTemplate:Idea template`, href: '/admin/idea-template', icon: 'document-text' },
  ...
  readonly canSubmitIdea = computed(() => this.hasAnyRole(SUBMITTER_ONLY));
