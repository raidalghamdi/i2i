...
public class EmailOutboxProcessEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly WebApplicationFactory<Program> _factory;

    public EmailOutboxProcessEndpointTests(WebApplicationFactory<Program> factory)
    {
    ...
    [Fact]
    public async Task AdminGetsZeroCountsForEmptyOutbox()
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Dev-User", "admin1");

        var response = await client.PostAsync("/api/admin/email-outbox/process", null);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadAsStringAsync();
        Assert.Contains("\"processed\":0", body);
        Assert.Contains("\"sent\":0", body);
