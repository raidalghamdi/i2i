...
public class AuditVerifyEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly WebApplicationFactory<Program> _factory;

    public AuditVerifyEndpointTests(WebApplicationFactory<Program> factory)
    {
    ...
    [Fact]
    public async Task AdminGetsValidResultForEmptyChain()
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Dev-User", "admin1");

        var response = await client.GetAsync("/api/admin/audit/verify");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadAsStringAsync();
        Assert.Contains("\"isValid\":true", body);
        Assert.Contains("\"brokenAtChainSeq\":null", body);
    }
