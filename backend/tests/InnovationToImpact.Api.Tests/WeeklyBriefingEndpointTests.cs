...
public class WeeklyBriefingEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly WebApplicationFactory<Program> _factory;

    public WeeklyBriefingEndpointTests(WebApplicationFactory<Program> factory)
    {
    ...
    [Fact]
    public async Task AdminGetsZeroCountsForEmptyTables_ButOneRecipientQueued()
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Dev-User", "admin1");

        var response = await client.PostAsync("/api/admin/briefing/generate", null);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadAsStringAsync();
        Assert.Contains("\"slaBreachesThisWeek\":0", body);
        Assert.Contains("\"invitationsAcceptedThisWeek\":0", body);
        Assert.Contains("\"pendingInvitations\":0", body);
        Assert.Contains("\"expiredInvitations\":0", body);
        Assert.Contains("\"auditEntriesThisWeek\":0", body);
        Assert.Contains("\"recipientsQueued\":1", body);
    }
