...
public class AuditLogReportEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly WebApplicationFactory<Program> _factory;
    private readonly string _rootPath = Path.Combine(Path.GetTempPath(), $"report-storage-test-{Guid.NewGuid():N}");

    public AuditLogReportEndpointTests(WebApplicationFactory<Program> factory)
    {
    ...
    [Fact]
    public async Task AdminGeneratesReport_ReturnsCompletedStatusWithFileUrl()
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Dev-User", "admin1");

        var response = await client.PostAsync("/api/admin/reports/audit-log/generate", null);
