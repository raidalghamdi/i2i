...
public class InvitationReminderEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly WebApplicationFactory<Program> _factory;

    public InvitationReminderEndpointTests(WebApplicationFactory<Program> factory)
    {
    ...
    [Fact]
    public async Task AdminGetsZeroCountsForEmptyInvitationTable()
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Dev-User", "admin1");

        var response = await client.PostAsync("/api/admin/invitations/remind", null);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadAsStringAsync();
        Assert.Contains("\"scanned\":0", body);
        Assert.Contains("\"expired\":0", body);
        Assert.Contains("\"remindersQueued\":0", body);
    }
