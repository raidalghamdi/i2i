...
public class UserManagementService : IUserManagementService
{
    ...
    public async Task<BulkAdImportResult> ImportAllAdUsersAsSubmittersAsync(Guid grantedById, int maxResults, CancellationToken cancellationToken = default)
    {
        var submitterRole = await _db.Roles.SingleOrDefaultAsync(r => r.Code == RoleCodes.Submitter, cancellationToken);
        if (submitterRole is null)
        {
            return new BulkAdImportResult(0, 0, 0, 0, 0);
        }

        var identities = await _lookupService.ListAllAsync(maxResults, cancellationToken);

        int granted = 0, pending = 0, skippedHasRoles = 0, skippedAlreadyPending = 0;
        foreach (var identity in identities)
        {
            var sam = identity.SamAccountName;
            ...
            var user = await _db.Users.Include(u => u.UserRoles)
                .SingleOrDefaultAsync(u => u.SamAccountName == sam, cancellationToken);

            if (user is not null)
            {
                // Only touch accounts that currently have no roles at all; never remove roles.
                if (user.UserRoles.Any())
                {
                    skippedHasRoles++;
                    continue;
                }
                user.UserRoles.Add(new UserRole { UserId = user.Id, RoleId = submitterRole.Id, Role = submitterRole, IsPrimary = false });
                granted++;
            }
            else
            {
                // Never-logged-in person: queue a pending submitter grant unless they're already queued
                // (for any role) — the grant materialises into a real user at their first login.
                var alreadyPending = await _db.PendingRoleGrants.AnyAsync(g => g.SamAccountName == sam, cancellationToken);
                if (alreadyPending)
                {
                    skippedAlreadyPending++;
                    continue;
                }
                _db.PendingRoleGrants.Add(new PendingRoleGrant
                {
                    Id = Guid.NewGuid(),
                    SamAccountName = sam,
        ...
        await _db.SaveChangesAsync(cancellationToken);
        return new BulkAdImportResult(identities.Count, granted, pending, skippedHasRoles, skippedAlreadyPending);
    }
